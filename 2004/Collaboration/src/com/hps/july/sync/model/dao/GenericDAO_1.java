...
public class GenericDAO_1 implements DbsPositionDAO_Page {
	...
	*/
	...
	public GenericDAO_1() throws ReferencesDAOSysException {
		try {
	...
	private GenericDAO_1(String daoSQLFileName, String database) throws ReferencesDAOSysException {
		try {
			SAXParserFactory parserFactory = SAXParserFactory.newInstance();
			parserFactory.setValidating(true);
			parserFactory.setNamespaceAware(true);
	...
	protected static DataSource getDataSource() throws ReferencesDAOSysException {
		try {
			InitialContext context = new InitialContext();
	...
	*/
	private PreparedStatement buildSQLStatement(Connection connection, Map sqlStatements, String sqlStatementKey, String[] parameterValues) throws SQLException {
	...
	private PreparedStatement buildSQLStatement(Connection connection, Statement sqlStatement, String[] parameterValues) throws SQLException {
	 ...
	 */
	public Page findDbsPositionsPage(Connection con, int start, int count, String orderBy) throws SQLException {
		//String[] parameterValues = new String[] { locale.toString()};
	...
	//основной метод:
	protected Page executeSelect(Connection con, String xml_get_pagem, String[] parameterValues, int start, int count, String orderBy) throws SQLException {
		PreparedStatement statement = null;
		ResultSet resultSet = null;
		try {
			if (TRACE) {
				printSQLStatement(sqlStatements, XML_GET_DbsPositions, parameterValues);
			}
			statement = buildSQLStatement(con, sqlStatements, XML_GET_DbsPositions, parameterValues);
			resultSet = statement.executeQuery();
			...
			if (start >= 0 && resultSet.absolute(start + 1)) {
				...
				return new Page(list, start, hasNext);
			}
			return Page.EMPTY_PAGE;
		} catch (SQLException exception) {
			throw new ReferencesDAOSysException("SQLException: " + exception.getMessage());
	...
	//populateRow:
	private Object populateRow(ResultSet rs, int numberOfColumns) throws SQLException {
